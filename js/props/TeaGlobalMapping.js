...
//
// !! IMPORTANT !!
// These mappings are detailed starting points based on common associations
// and descriptions found in your project files. You should review, test,
// and refine these values based on your specific research, goals,
// and observed results from your calculator.

// --- Mappings from Tea Data -> TCM Properties ---
...
export const mapCompoundRatioAndProcessingToYinYangScore = (tea) => {
   ...
   * Uses reliable Geography (mandatory Lat, Lon, Alt; optional Climate, Features, Soil, Sunlight, Humidity)
   * and reliable Processing data.
   */
  export const mapGeographyAndProcessingToElementScores = (tea) => {
    ...
    const effectiveClimate = climate || 'temperate'; // Use a default if null/undefined for logic flow
    ...
    // 8. Processing Influence (Reliable)
    // Example - needs integration with FiveElements.js mappings or similar structure
     ...
     // Add more mappings based on fiveElementAssociations.processing
  
     // 9. Flavor Influence (Optional - Dominant flavor)
     // Example - needs integration with FiveElements.js mappings or similar structure
     /*
     const primaryFlavor = flavorProfile?.[0];
     if (primaryFlavor && fiveElementAssociations.flavors[primaryFlavor]) {
         const flavorAssociations = fiveElementAssociations.flavors[primaryFlavor];
         Object.entries(flavorAssociations).forEach(([element, strength]) => {
             elementScores[element] += strength * 0.8;
         });
     }
     */
  ...
  export const mapCompoundsAndProcessingToQiMovementScores = (tea) => {
      ...
      // 2. Processing Influence (Reliable)
      if (processingMethods.includes('heavy-roast') || processingMethods.includes('charcoal-roasted') || processingMethods.includes('aged') || processingMethods.includes('fermented') || processingMethods.includes('compressed')) {
          qiScores.descending += 1.5; qiScores.contracting += 1.0;
      }
      if (processingMethods.includes('steamed') || processingMethods.includes('minimal-processing') || processingMethods.includes('light-oxidation') || processingMethods.includes('sun-dried')) {
          qiScores.rising += 1.2; qiScores.expanding += 0.6;
      }
      if (processingMethods.includes('rolled')) {
         qiScores.contracting += 0.6;
      }
       if (processingMethods.includes('partial-oxidation') || processingMethods.includes('gaba-processed') || processingMethods.includes('medium-roast')) {
         qiScores.balanced += 1.2;
      }
      // Add more based on QiMovement.js mappings for processing
  
      return qiScores;
  };
  
  
  // --- Mapping: TCM Property -> Primary Effect (Detailed Starting Point) ---
  
  export const tcmToPrimaryEffectMap = {
    // Yin/Yang Nature -> Primary Effects ([Effect Name, Strength])
    Yin: [
      ['soothing', 3.0], ['peaceful', 2.5], ['restorative', 1.5], ['reflective', 1.0], ['centering', 0.5]
    ],
    Yang: [
      ['revitalizing', 3.0], ['awakening', 2.5], ['nurturing', 1.5], ['elevating', 1.0], ['energizing', 1.0]
    ],
    Balanced: [
    ...
    earth: [
      ['balancing', 2.5], ['nurturing', 2.0], ['stabilizing', 1.5], ['comforting', 1.5], ['centering', 1.0]
    ],
    metal: [
      ['clarifying', 2.5], ['reflective', 2.0], ['balancing', 1.0], ['soothing', 0.5] // Removed contracting, might be redundant w/ Qi
    ],
    water: [
      ['centering', 2.5], ['stabilizing', 2.0], ['reflective', 1.5], ['peaceful', 1.5], ['restorative', 1.0]
    ],
    ...
    descending: [
      ['centering', 3.0], ['stabilizing', 2.5], ['peaceful', 2.0], ['soothing', 1.5], ['grounding', 1.0] // Added grounding
    ],
    expanding: [
    ...
    balancedQi: [ // Renamed from 'balanced'
