...
export async function generateEffectsReport(progressCallback) {
    // Initialize calculator with proper EffectSystemConfig instance
    ...
    for (const tea of teaDatabase) {
        ...
        try {
            ...
            const finalEffects = result.data?.finalScores || {};
            const baseEffects = result.data?.componentScores?.base || {};
            
            const topFinalEffects = getTopEffects(finalEffects);
            const topBaseEffects = getTopEffects(baseEffects);
            
            const teaTypeEffects = result.data?.componentScores?.teaType || {};
