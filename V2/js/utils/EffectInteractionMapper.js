...
function generateModifiers(effect1, effect2, combination) {
  ...
  if (name.includes('vitality') || description.includes('energy') || description.includes('alertness')) {
    modifies.push({ target: 'energizing', modifier: 0.3 });
  }
  ...
  if (name.includes('peace') || name.includes('tranquil') || description.includes('calm') || description.includes('relax')) {
    modifies.push({ target: 'calming', modifier: 0.4 });
    modifies.push({ target: 'restorative', modifier: 0.3 });
  }
  ...
  if (name.includes('ground') || name.includes('stable') || description.includes('anchor')) {
    modifies.push({ target: 'grounding', modifier: 0.4 });
  }
  
  // Comfort combinations
  if (name.includes('comfort') || name.includes('nurturing') || description.includes('support')) {
    modifies.push({ target: 'comforting', modifier: 0.4 });
  }
